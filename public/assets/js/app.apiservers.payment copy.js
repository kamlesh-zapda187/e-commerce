$(function () {
	...
	const CONFIG = {
		...
		servers: {
			...
			apiBaseUrl: () => {
				...
				/*
				if (url.hostname.includes("development")) {
					// Development url 
					return `${CONFIG.servers.dev}/api`
				} else if (url.hostname.includes("marketplace")) {
					// production url 
					return `${CONFIG.servers.prod}/api`
				} else {
					return `${CONFIG.servers.local}/api`
				}
				*/
			...
			mountStripeForm: formId => {
				...
				$(formId).on("submit", async function (e) {
					...
					const { error } = await stripe.confirmPayment({
						...
						confirmParams: {
							/* 
							***PLEASE DON'T CHANGE THE WINDOW.LOCATION.HREF AGAIN!***
							
							By removing the the window.location.href the payment will not be captured at the backed level which is bad business.
							they is a method at the button of this code the makes sure that the page is redirect to success page. */
							return_url: `${window.location.href}/processing?order=${sessionStorage.getItem(
								"orderId"
							)}`
						}
					})
	...
	if (HTTPClient) {
		...
		const Apis = {
			 ...
			 * @param {string} orderId The product order ID
			 */
			async verifyOrder(orderId, params) {
				try {
					$("#preloader-custom").removeClass("d-none")
					if (orderId && params) {
						/*						console.log("ffdsfsf trest");						alert("test function 1");
						$("#preloader-custom").removeClass("d-none");						*/ /*$(document).trigger("loader_on")*/

						const { data } = await POST(`/store/order/${orderId}/verify`, params)
							.then(res => {
								/*$(document).trigger("loader_off") */
								/* add loader here */ //alert("test function 2");
								if (res.status) {
									Toast().fire({
										type: "success",
										title: res?.message
		...
		const { CSRF, placeOrder, verifyOrder } = Apis
		...
		function handlePaymentSuccess() {
			...
			if (params.get("order") && params.get("payment_intent")) {
				const paymentInfo = {
				...
				// this
				setTimeout(() => {
					verifyOrder(params.get("order"), paymentInfo)
						.then(res => {
							if (res) {
								sessionStorage.clear()

								const success = window.location.href.replace(
									"product/checkout/processing",
									"product/orderSuccess"
								)

								setTimeout(() => window.location.replace(success), 100)
							}
						})
						.catch(err => {
							/* Show the preloader */
